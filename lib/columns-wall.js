        ...
        updateLayout: function (config) {
            var me = {
                ...
                doLayout: function (index) {
                    ...
                    me.layout(image, function (actualWidth, actualHeight) {
                        ...
                        try {
                            ...
                            me.last[pos] = li;
                            li.css({left: x, top: y}).addClass(STATE_MEASURED).removeClass(STATE_PROCESSING).addClass('__col_' + pos).css({opacity: 1});
                            me.doLayout(index + 1);
                        } catch (e) {
                            alert([e,actualWidth,actualHeight].join(','));
                        }
                    });
                },
                layout: function (image, callback) {
                    ...
                    var ie = ('v' == '\v');
                    measured.src = url+(ie?(url.match(/\?/)?'&':'?')+'rnd='+(+new Date):'');
                    var done = false;
