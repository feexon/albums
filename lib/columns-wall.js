(function () {
    ...
    function doLayout(pos) {
        ...
        for (var k = 0; k < columns; k++) {
            ...
            if (!candidate.length) {
                position = k;
                break;
            } else if (candidate.position().top + candidate.outerHeight() < low.position().top + low.outerHeight()) {
                position = k;
            }
        }
        ...
        if (prev.hasClass(MEASURED) && top.hasClass(MEASURED)) {
            if (!post[pos]) {
                post[pos] = true;
                var x = prev.position().left + prev.outerWidth();
                var y = top.position().top + top.outerHeight();
    ...
    wall.updateLayout = function () {
        ...
        columns = parseInt(owner.width() / thumbs.outerWidth());
        post={};
